...
"""
Assign product code and set launch permission
...
def share_public(ami_id, region):
    ...
    conn.modify_image_attribute(
        ami_id,
        product_codes=['BA39B70F'])

    log.debug('setting product launch permissions - %s', ami_id)
    conn.modify_image_attribute(
        ami_id,
    ...
    log.info('set image to product code and launch permissions - %s' % ami_id)
